...
class FileManager():
    ...
    def findMainJavaFile(self):
        ...
        for root, dirs, files in os.walk(self.projectPath):
            for file in files:
                if (re.search(r".*java", file) and file != self.mainPath):
                    newPath = root + "/" + file
                    break
        return newPath

    def getMainFromProjectSetting(self):
        newPath = ''
        classpathFile = self.projectPath + "/.classpath"
        if (os.path.isfile(classpathFile)):
            return ''
    ...
    def updateJavaFile(self):
        ...
        if (self.mainPath == ''):
            ...
            if (self.studentID == ''):
                ...
                print("EXIT!")
                exit()
            self.mainPath = '{}/{}_{}.java'.format(self.projectPath, self.projectName, self.studentID)
        else:
            os.remove(self.mainPath)
        
        shutil.copy(newPath, self.mainPath)
